...
1. **Create a Reproducible Verification Case**
   - Capture the simplest possible reproduction before applying the fix
   - Use an automated regression test when the behavior is suitable for automated testing
   - Use a one-off diagnostic script, command, readback, or documented reproduction when no test framework applies
   - Confirm that the reproduction demonstrates the original failure before fixing it whenever feasible
...
3. **Verify Fix**
   ...
   - Use the `yiming-verification-before-completion` skill before claiming success
...
| **4. Implementation** | Capture reproduction, fix root cause, verify | Bug resolved, verification passes |
